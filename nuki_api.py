...
class Nuki:
    ...
    def setKey(self, id: int, beginDate: datetime.date, endDate: datetime.date, checkin: datetime.time, checkout: datetime.time, name: str = "pin", debug: bool = False, pin: str = None) -> Tuple[str, bool]:
        ...
        start = datetime.datetime(beginDate.year, beginDate.month,
                                  beginDate.day, checkin.hour, checkin.minute, checkin.second)
        end = datetime.datetime(endDate.year, endDate.month, endDate.day,
                                checkout.hour, checkout.minute, checkout.second)
    ...
    def getPin(self, lock_id: int, pin_name: str, debug: bool = False) -> Pin:
        ...
        header = {"Authorization": self.api_key, "Accept": "application/json"}
        req = requests.get(self.api_url + "smartlock/" +
                           str(lock_id) + "/auth", headers=header)
        json = req.json()
        for key in json:
            if key["name"] == pin_name:
                ...
                return Pin(key["id"], key["smartlockId"], key["authId"], int(key["type"]), key["name"], bool(key["enabled"]), bool(key["remoteAllowed"]), int(key["lockCount"]), lastActiveDate, creationDate, updateDate, code, key)
        return None
    ...
    def getAllPins(self, lock_id: str) -> List[Pin]:
        ...
        for pin in json:
            ...
            p = Pin(pin["id"], pin["smartlockId"], pin["authId"], int(pin["type"]), pin["name"], bool(pin["enabled"]), bool(
                pin["remoteAllowed"]), int(pin["lockCount"]), lastActiveDate, creationDate, updateDate, code, pin)
            pins.append(p)
        return pins
    ...
    def deletePin(self, smartlock_id: str, pin_id: str) -> bool:
        ...
        header = {"Authorization": self.api_key, "Accept": "application/json"}
        req = requests.delete(self.api_url + "smartlock/" +
                              smartlock_id + "/auth/" + pin_id, headers=header)
        return req.ok
    ...
    def deactivatePin(self, smartlock_id: str, pin_id: str) -> Tuple[bool, list]:
        ...
        data = {"enable": False}
        req = requests.post(self.api_url + "smartlock/" + smartlock_id +
                            "/auth/" + pin_id, headers=header, data=data)
        return req.ok
    ...
    def updatePin(self, smartlock_id: str, pin_id: str, new_values: dict) -> Tuple[bool, list]:
        ...
        header = {"Authorization": self.api_key,
                  "Accept": "application/json", "Content-Type": "application/json"}
        req = requests.post(self.api_url + "smartlock/" + smartlock_id +
                            "/auth/" + pin_id, headers=header, data=json.dumps(new_values))
